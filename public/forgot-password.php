...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!validateCSRFToken($csrf_token)) {
            ...
            $stmt = $pdo->prepare("SELECT student_id, name, email FROM students WHERE email = ? AND is_active = 1");
            $stmt->execute([$email]);
            ...
            if ($student) {
                ...
                // Store OTP in database (using token field)
                $stmt = $pdo->prepare("
                ...
                try {
                    error_log("[FORGOT_PASSWORD] Creating EmailService...");
                    $emailService = new EmailService();
                    error_log("[FORGOT_PASSWORD] EmailService created successfully");
                    
                    error_log("[FORGOT_PASSWORD] Sending OTP to: {$email}");
                    $result = $emailService->sendPasswordResetOTP($email, $student['name'], $otp, 10);
                    error_log("[FORGOT_PASSWORD] Send result: " . json_encode($result));
                    ...
                    if ($result['success']) {
                        ...
                        $error_message = "Failed to send OTP: " . ($result['message'] ?? 'Unknown error');
                        error_log("[FORGOT_PASSWORD] OTP send failed: " . $result['message']);
                    }
                } catch (Exception $e) {
                    $error_message = "Email service error: " . $e->getMessage();
                    error_log("[FORGOT_PASSWORD] Email service exception: " . $e->getMessage());
...
<head>
    ...
    <meta name="description" content="Reset your password for QR Attendance System" />
